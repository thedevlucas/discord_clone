...
import { friends, dmMessages, currentUser } from "@/lib/data";
...
function AtIcon({ className }: { className?: string }) {
  return (
    <svg className={className} aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
      <path fill="currentColor" d="M12.5 2a10 10 0 1 0 4.67 18.85.75.75 0 0 0-.68-1.34A8.5 8.5 0 1 1 21 11.5a2.5 2.5 0 0 1-5 0V8a.75.75 0 0 0-1.5 0v.25a4 4 0 1 0 .8 6.19A4 4 0 0 0 22.5 11.5 10 10 0 0 0 12.5 2Zm2.5 9.5a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0Z"/>
    </svg>
  );
}

export default function DMChat({ friendId }: DMChatProps) {
  const friend = friends.find(f => f.id === friendId);
  const messages = dmMessages[friendId] || [];
  ...
  const processedMessages = messages.map((msg, idx) => ({
    ...
    sender: msg.senderId === currentUser.id ? currentUser : friend,
  }));
    ...
    <div className={styles.chatArea}>
      ...
      <header className={styles.channelHeader}>
        <div className={styles.channelInfo}>
          <AtIcon className={styles.channelIcon} />
          <h1 className={styles.channelName}>{friend.username}</h1>
        ...
        <div className={styles.headerActions}>
          ...
          <button className={styles.headerActionBtn} aria-label="Show User Profile">
            <svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
      ...
      <div className={styles.messagesWrapper}>
        <div className={styles.messagesScroller}>
          ...
          <div className={styles.dmWelcome}>
            ...
            />
            <h2 className={styles.welcomeTitle}>{friend.username}</h2>
            <p className={styles.welcomeDescription}>
              Este es el inicio de tu historial de mensajes directos con <strong>{friend.username}</strong>.
            </p>
            <div className={styles.welcomeActions}>
              <button className={styles.welcomeActionBtn}>
              ...
              <button className={styles.welcomeActionBtn}>
                <svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M18.4 4L12 10.4 5.6 4 4 5.6 10.4 12 4 18.4 5.6 20 12 13.6 18.4 20 20 18.4 13.6 12 20 5.6 18.4 4Z"/>
                </svg>
                Bloquear
              </button>
            </div>
                    ...
                    <div className={styles.messageContent}>
                      <div className={styles.messageHeader}>
                        <span className={styles.authorName}>
                          {msg.senderId === currentUser.id ? currentUser.username : friend.username}
                        </span>
        ...
        <div className={styles.messageInputWrapper}>
          ...
          <div className={styles.inputContainer}>
            <input 
              ...
              aria-label={`Enviar mensaje a @${friend.username}`}
            />
          </div>
          <div className={styles.inputButtons}>
