      ...
      <ul className={styles.serverList}>
        ...
        <li className={styles.serverItem}>
          ...
          <div className={styles.serverIconWrapper}>
            <div className={styles.serverIcon} data-home="true">
        ...
        {servers.slice(1).map((server) => (
          <li key={server.id} className={styles.serverItem}>
            <div className={styles.pillWrapper}>
              {(server.isSelected || server.hasNotification) && (
                <span 
                  className={`${styles.pill} ${server.isSelected ? styles.pillSelected : styles.pillNotification}`}
                ></span>
              )}
            </div>
            <div 
              className={`${styles.serverIconWrapper} ${server.isSelected ? styles.selected : ""}`}
              data-has-notification={server.hasNotification}
            >
              {server.icon ? (
                <div 
                  className={`${styles.serverIcon} ${server.isSelected ? styles.serverIconSelected : ""}`}
                  style={{ 
